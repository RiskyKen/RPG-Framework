...
import moe.plushie.rpg_framework.core.client.helper.ModRenderHelper;
...
public class GuiTextFieldCustom extends GuiButtonExt {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partial) {
        ...
        if (this.getText().trim().isEmpty()) {
            ...
            if (s.length() > 0 & !isFocused()) {
                mc.fontRenderer.drawStringWithShadow(emptyLabel, x + 4, y + 4, 0xFF7F7F7F);
            }
        }
        ...
        mc.fontRenderer.drawSplitString(s, x + 5, y + 5, width - 8, 0x444444);
        mc.fontRenderer.drawSplitString(s, x + 4, y + 4, width - 8, 0xFFFFFF);
    ...
    public boolean keyTyped(char c, int keycode) {
        ...
        case Keyboard.KEY_V:
            if (GuiScreen.isCtrlKeyDown()) {
                RPGFramework.getLogger().info(GuiScreen.getClipboardString());
