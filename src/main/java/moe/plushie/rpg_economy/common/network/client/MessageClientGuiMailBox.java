package moe.plushie.rpg_economy.common.network.client;
...
import moe.plushie.rpg_economy.common.mail.serialize.MailMessageSerializer;
...
public class MessageClientGuiMailBox implements IMessage, IMessageHandler<MessageClientGuiMailBox, IMessage> {
    ...
    public void toBytes(ByteBuf buf) {
        ...
        RPG_Economy.getLogger().info("Sending json to server. " + jsonElement.toString());
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        RPG_Economy.getLogger().info("Got json from client. " + mailJson);
    ...
    public IMessage onMessage(MessageClientGuiMailBox message, MessageContext ctx) {
        RPG_Economy.getProxy().getMailSystemManager().onClientSendMailMessage(ctx.getServerHandler().player, message.mailMessage);
        return null;
    }
}
